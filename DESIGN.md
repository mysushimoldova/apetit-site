...
на 1.5 размытия с каждой стороны. Числа — `src/config/motion.json` → `products.shadow`,
вкладка «Produse» в `/dev/motion`.
...
- Карточки блюд при первом появлении в кадре: двигаются только фото и тень,
  ...
  opacity 0→1 и scaleX .8→1, на 100 мс позже. 360 мс, `cubic-bezier(.22,1,.36,1)`,
...
- **2.7 · 23.09.2026** — «живые» карточки блюд. Плоскую серую лужицу под фото
  ...
  только фото и тень, 360 мс, задержка по колонкам 70 мс. При прокрутке фото
