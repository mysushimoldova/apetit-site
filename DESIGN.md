...
**Версия:** 2.3 · 19.09.2026 · утверждено по макетам docs/mock-*.png · собрана по референсам Амяна (см. ниже) + бренд Apetit из PSD меню
...
точках): исходник — видео `assets/brand/linii-fundal.mp4` (4K, серые линии на
белом; отдельной картинки нет). Из него берётся один кадр (8-я секунда) и
скриптом `scripts/bg-lines.py` превращается в `public/img/bg/linii.webp`:
только линии, цвет **Ash #A79E95**, фон прозрачный, 1600×900, ~52 КБ.
- **Толщина:** линии перерисованы тонкими — **~1px на экране телефона 390px**
  (вариант A1; параметр `--line` скрипта, 0.6 — «волосок» A2).
- **Сила:** весь слой с `opacity: 0.5` (переменная `--bg-lines-opacity`,
  вариант A).
- **Слой:** экран городов, меню, оформление; `position: fixed`, под всем
  контентом (z-index −1), `pointer-events: none`. Картинка — `cover` на высоту
  экрана; в слое три экрана подряд: картинка, её зеркальная копия, картинка —
  стыков не видно.
- **Движение:** при скролле слой едет вверх на **0.35× прокрутки** (только
  transform, Motion `useScroll`), через каждые два экрана сдвиг начинается
  заново. Стоит, когда не скроллят; при reduced-motion — неподвижен.
