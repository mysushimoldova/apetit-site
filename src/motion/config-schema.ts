...
export const splashSchema = z.strictObject({
  ...
  skip: z.boolean(),
  /** Сколько роликов держать «тёплыми» — с готовым первым кадром (решение
   *  архитектора 25.09.2026). Каждый тёплый ролик — свой декодер в
   *  видеокарте. Файлы скачиваются для всех категорий точки, а греются
   *  только чипы, видные в ленте, и последняя открытая категория. */
  warmMax: splashRange("warmMax").int(),
});
