...
describe("правила кеша", () => {
  ...
  // В разработке имя куска сборки не меняется при правке кода. С годовым
  // immutable телефон Амяна брал из кеша старый кусок с настройками
  // заставки вперемешку с новым кодом — и заставка вешала страницу
  // (25.09.2026). Поэтому в dev правила для сборки нет, остальное — как в бою.
  it("в разработке сборка не кешируется надолго, остальное как в бою", () => {
    const build = "/_next/static/:path*";
    const dev = cacheRules(true);
    expect(dev.map((r) => r.source)).not.toContain(build);
    expect(dev).toEqual(cacheRules().filter((r) => r.source !== build));
  });
